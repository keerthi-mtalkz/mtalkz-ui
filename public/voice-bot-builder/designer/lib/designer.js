(function (global, factory) {
	...
	class SequenceModifier {
	    ...
	    static insertStep(step, sequence, index) {
	        sequence.splice(index, 0, step);
	    }
	    static deleteStep(step, sequence) {
	...
	class DesignerContext {
	    ...
	    deleteStepById(stepId) {
	        ...
	        if (((_a = this.selectedStep) === null || _a === void 0 ? void 0 : _a.id) === stepId) {
	...
	class GlobalEditor {
	    ...
	    static create(definition, globalEditorProvider) {
	...
	class StepEditor {
	    ...
	    static create(step, stepEditorProvider) {
	...
	class TaskStepComponentView {
	    ...
	    static create(parent, step, configuration) {
	        const g = Dom.svg('g', {
	            class: `sqd-task-group sqd-task-${step.type}`
	        });
	        parent.appendChild(g);
	...
	class DragStepBehavior {
	    constructor(view, context, step, pressedStepComponent) {
	        this.view = view;
	        this.context = context;
	        this.step = step;
	        this.pressedStepComponent = pressedStepComponent;
	    }
	    static create(context, step, pressedStepComponent) {
	    ...
	    onEnd(interrupt) {
	        ...
	        if (!interrupt && this.currentPlaceholder) {
	            if (this.pressedStepComponent) {
	                SequenceModifier.moveStep(this.pressedStepComponent.parentSequence, this.pressedStepComponent.step, this.currentPlaceholder.parentSequence, this.currentPlaceholder.index);
	            }
	            else {
